...
ggplot(data, aes(x = AMT_INCOME_TOTAL, fill = factor(TRUE))) +
  ...
  xlab(label = "Annual income") +
  ggtitle("Scaled annual income distribution")
```
...
```{r}
qqnorm(data$AMT_INCOME_TOTAL, main="QQ plot of normal data", pch="O", col=LIGHT_PURPLE)
qqline(data$AMT_INCOME_TOTAL, lty=1, lwd=3, col="white")
qqline(data$AMT_INCOME_TOTAL, lty=2, lwd=3, col=LIGHT_PINK)
```

```{r}
par(mfrow=c(2,2))

qqPlot(data$AMT_INCOME_TOTAL, "norm",  pch="*", col=LIGHT_PURPLE, col.lines=DARK_PINK)
qqPlot(data$AMT_INCOME_TOTAL, "lnorm", pch="*", col=LIGHT_PURPLE, col.lines=DARK_PINK)
...
=======
>>>>>>> bf91ef51d5b71d6c5aa2c4e421968805ff27dbad
```
